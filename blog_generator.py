...
def generate_article(row, template, all_articles=None):
    ...
    # Related domains are injected later by
    # inject_related_domains.py using data/domains.json.
    page_content = page_content.replace("{{RELATED_DOMAINS}}", "")
...
def inject_related_into_article(slug, all_articles):
    ...
    content = content.replace("{{RELATED_DOMAINS}}", "")
